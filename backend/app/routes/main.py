...
def generate_story():
    ...
    model_name = current_app.config.get('SILICONFLOW_MODEL_NAME')
    ...
    if not model_name:
        return jsonify({"error": "服务器配置缺失: SILICONFLOW_MODEL_NAME 未设置"}), 500
    # --- 配置校验结束 ---
    ...
    stream = generate_life_story_stream(country_data, api_key, api_base, model_name)
